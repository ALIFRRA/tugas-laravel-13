...
<body class="font-sans min-h-screen bg-shuka-blush">
    <div class="min-h-screen lg:flex" x-data="{ open: false }">
        <div class="lg:hidden flex items-center justify-between border-b border-shuka-line bg-white/90 px-4 py-3">
            <a href="{{ route('guru.dashboard') }}" class="font-display text-2xl text-shuka-pink">Shuka Highschool</a>
            <button type="button" @click="open = !open" class="border border-shuka-line px-3 py-1.5 text-sm text-slate-600">Menu</button>
        </div>
        ...
        >
            <div class="flex h-full flex-col bg-strings">
                <div class="border-b border-shuka-line px-5 py-5">
                    ...
                    <p class="mt-2 text-xs text-shuka-muted">area guru</p>
                </div>

                <nav class="flex-1 space-y-0.5 overflow-y-auto px-3 py-4 text-sm">
                    <a href="{{ route('guru.dashboard') }}"
                       class="block border-l-2 px-3 py-2 {{ request()->routeIs('guru.dashboard') ? 'border-shuka-pink bg-shuka-soft/70 text-shuka-pink' : 'border-transparent text-slate-600 hover:border-shuka-string hover:bg-slate-50' }}">
                        Dashboard
                    </a>
                    <a href="{{ route('guru.nilai.index') }}"
                       class="block border-l-2 px-3 py-2 {{ request()->routeIs('guru.nilai.*') ? 'border-shuka-pink bg-shuka-soft/70 text-shuka-pink' : 'border-transparent text-slate-600 hover:border-shuka-string hover:bg-slate-50' }}">
                        Nilai
                    </a>
        ...
        <div class="fixed inset-0 z-30 bg-slate-900/10 lg:hidden" x-show="open" x-transition.opacity @click="open = false" style="display: none;"></div>
            ...
            <header class="border-b border-shuka-line bg-white/95 px-4 py-4 sm:px-6 lg:px-8">
                <div class="flex items-center justify-between gap-4">
                    ...
                    <a href="{{ route('profile.show', Auth::id()) }}" class="flex items-center gap-2 border border-shuka-line bg-shuka-blush px-2 py-1.5 hover:border-shuka-pink sm:px-3">
                        ...
                        <span class="hidden max-w-[9rem] truncate text-sm font-medium text-slate-700 sm:inline">{{ Auth::user()->name }}</span>
                    </a>
                </div>
            </header>
