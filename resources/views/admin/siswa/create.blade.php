...
@section('title', 'Tambah Siswa — Shuka Highschool')
@section('heading', 'Tambah siswa')
@section('subheading', 'Isi data siswa baru.')

@section('content')
    <form method="POST" action="{{ route('admin.siswa.store') }}" class="soft-panel max-w-2xl space-y-5 p-5 sm:p-6">
        @csrf
        <x-input name="nama" label="Nama" :value="old('nama')" required />
        <div class="grid gap-5 sm:grid-cols-2">
            <x-input name="nis" label="NIS" :value="old('nis')" required />
        ...
        <x-input type="textarea" name="alamat" label="Alamat" :value="old('alamat')" required rows="3" />
