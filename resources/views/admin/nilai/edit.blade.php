...
@section('title', 'Edit Nilai — SMK Shuka')
@section('heading', 'Perbarui Nilai Siswa')

@section('content')
<div class="max-w-2xl">
    <div class="bg-white border border-slate-200 rounded-lg p-6 shadow-2xs space-y-5">
        <div class="border-b border-slate-200 pb-3">
            <h3 class="text-sm font-bold text-slate-900">Perbarui Catatan Nilai</h3>
            <p class="text-xs text-slate-500">Ubah data penilaian siswa sesuai perbaikan hasil evaluasi.</p>
        </div>

        <form method="POST" action="{{ route('admin.nilai.update', $nilai) }}" class="space-y-4">
            @csrf
            @method('PUT')

            <!-- komponen pencarian siswa -->
            <x-siswa-picker :siswas="$siswas" label="Pilih Siswa" :selected="old('siswa_id', $nilai->siswa_id)" />
            ...
            <div>
                ...
                <select name="mapel_id" required class="w-full text-xs rounded border-slate-300 focus:border-pink-500 focus:ring-pink-500 py-2 px-3 bg-white text-slate-900">
                    ...
                    @foreach ($mapels as $mapel)
                        <option value="{{ $mapel->id }}" @selected((string) old('mapel_id', $nilai->mapel_id) === (string) $mapel->id)>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="jenis_nilai" required class="w-full text-xs rounded border-slate-300 focus:border-pink-500 focus:ring-pink-500 py-2 px-3 bg-white text-slate-900">
                        <option value="Tugas" @selected(old('jenis_nilai', $nilai->jenis_nilai) === 'Tugas')>Tugas Praktik / Studio</option>
                        <option value="UH" @selected(old('jenis_nilai', $nilai->jenis_nilai) === 'UH')>Ulangan Harian (UH)</option>
                        <option value="UTS" @selected(old('jenis_nilai', $nilai->jenis_nilai) === 'UTS')>Ujian Tengah Semester (UTS)</option>
                        <option value="UAS" @selected(old('jenis_nilai', $nilai->jenis_nilai) === 'UAS')>Ujian Akhir Semester (UAS)</option>
                        <option value="UKK" @selected(old('jenis_nilai', $nilai->jenis_nilai) === 'UKK')>Uji Kompetensi Kejuruan (UKK)</option>
                    </select>
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-700 mb-1">Skor Nilai (0 - 100) <span class="text-rose-500">*</span></label>
                    <input type="number" name="nilai" value="{{ old('nilai', $nilai->nilai) }}" required step="0.01" min="0" max="100" class="w-full text-xs rounded border-slate-300 focus:border-pink-500 focus:ring-pink-500 py-2 px-3">
                    @error('nilai')
                        <p class="text-xs text-rose-600 mt-1 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
