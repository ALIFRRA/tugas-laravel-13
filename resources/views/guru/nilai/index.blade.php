...
@section('heading', 'Nilai mapelku')
@section('subheading', 'Input dan analisis nilai murid.')
...
@section('content')
    <div class="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <x-button href="{{ route('guru.nilai.create') }}">Tambah nilai</x-button>
        </div>
    </div>
    ...
    <x-table :headers="['Siswa', 'Mapel', 'Jenis', 'Nilai', 'Aksi']">
            ...
            <tr>
                ...
                <td class="px-4 py-3">
                    <div class="flex flex-wrap gap-2">
                        <a href="{{ route('guru.nilai.edit', $nilai) }}" class="text-sm text-shuka-pink hover:underline">Edit</a>
                        <form action="{{ route('guru.nilai.destroy', $nilai) }}" method="POST" onsubmit="return confirm('Hapus nilai ini?')">
                            @csrf
                            @method('DELETE')
        ...
        @empty
            <tr>
                <td colspan="5" class="px-4 py-8 text-center text-slate-400">Belum ada nilai di mapelmu.</td>
            </tr>
        @endforelse
    </x-table>
